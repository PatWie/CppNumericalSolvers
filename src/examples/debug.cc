...
struct LinearFunction : public FunctionCRTP<LinearFunction, double,
                                            DifferentiabilityMode::First> {
...
int main() {
  ...
  // Test LinearFunction via AnyFunction (First-order).
  {
    VectorType m(2);
    m << 2, -1;
    double b_val = 0.5;
    LinearFunction lin(m, b_val);
    AnyFunction anyLin(lin);
  ...
  // Test ConstantFunction via AnyFunction (None).
  {
    ConstantFunction cf(3.14);
    AnyFunction anyConst(cf);
    ...
    AnyFunction negAnyConst = -anyConst;
    f_const = negAnyConst(x);
    std::cout << "Constant function value: " << f_const << "\n\n";
  }

  // Test QuadraticFunction via AnyFunction (Second-order).
  {
    MatrixType A(2, 2);
    A << 3, 1, 1, 2;
    VectorType b(2);
  ...
  // Test QuadraticFunction2 via AnyFunction (Second-order).
  {
  ...
  // Expression: (quad + lin - lin * 2.0)
  {
    ...
    AnyFunction prodFunc(quad * quad);
    VectorType grad2(2);
    quad(x, &grad2);
...

  {
    ...
    b << 1, -1;
    TScalar c_val = 0.5;
    AnyFunction objective = QuadraticFunction(A, b, c_val);
    ...
    m1 << 2, 1;
    TScalar b1 = -3;
    AnyFunction eqConstraint = LinearFunction(m1, b1);
    ...
    m2 << 1, 0;
    TScalar b2 = -1;
    AnyFunction ineqConstraint = LinearFunction(m2, b2);

    // Build the optimization problem.
    // ConstrainedOptimizationProblem<TScalar, DifferentiabilityMode::Second,
    //                     DifferentiabilityMode::First>
    //     prob(objective, {eqConstraint});
    ConstrainedOptimizationProblem prob(objective, { AnyFunction(-eqConstraint)},
                                        {ineqConstraint});
    // prob.objective = objective;
    // prob.initialState = VectorType(2);
    // prob.initialState << 0, 0;
    //
    // // Add constraints with their types, multipliers, and penalties.
    // // For instance, constraint 0 is equality; constraint 1 is inequality
    // (≤). prob.constraints.push_back(
    //     Constraint<TScalar, DifferentiabilityMode::First>(
    //         eqConstraint, ConstraintType::Equality, 1.0, 100.0));
    // prob.constraints.push_back(
    //     Constraint<TScalar, DifferentiabilityMode::First>(
    //         ineqConstraint, ConstraintType::InequalityLe, 2.0, 150.0));
    //
    // // Convert the constrained problem into an unconstrained augmented
    // // Lagrangian.
    ...
    AnyFunction unconstrainedPenalty = ToPenalty(prob, p_state);
    AnyFunction unconstrainedAugmentedLagrangian =
        ToAugmentedLagrangian(prob, l_state, p_state);
    VectorType x(2);
    x << 1, 3;

    AnyFunction penalty_part = FormPenaltyPart(prob, p_state);
    AnyFunction l_part = FormLagrangianPart(prob, l_state);

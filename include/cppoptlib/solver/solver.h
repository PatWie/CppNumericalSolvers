...
template <class FunctionType, class StateType> auto PrintCallback() {
...
template <class FunctionType, class StateType> auto NoOpCallback() {
  return [](const FunctionType & /*function*/, const StateType & /*state*/,
            const Progress<FunctionType, StateType> & /*progress*/) {};
}

// Specifies a solver implementation (of a given order) for a given function
template <typename FunctionType, typename StateType> class Solver {
public:
...
public:
  ...
  virtual std::tuple<StateType, Progress<FunctionType, StateType>>
  Minimize(const FunctionType &function, const StateType &function_state) {
